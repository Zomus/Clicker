module.exports = {
  drawCircle: function(ctx, x, y, radius, color = 0){
...
function drawRotated(ctx, x, y, rotation, drawFunction, ...drawParams){
...
function setFillStyle(ctx, color = 0){
  ctx.beginPath();
  //begin drawing
...
function setLineStyle(ctx, thickness = 1, color = 0, dashProp = [], lineCap = 'butt'){
  ctx.beginPath();
  //begin drawing
...
function drawCross(ctx, x, y, size = 25, thickness = 1, color = 0, lineFraction = 0.5, lineCap = 'butt'){
  ctx.save();
...
function drawCircle(ctx, x, y, radius, color = 0){
  ctx.save();
  //save canvas settings
